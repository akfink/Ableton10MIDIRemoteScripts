...
if g_mapper:
    ...
    class PrEditorMapper():

        def __init__(self, provider = None, root_path = None, num_controls = 8, snapshot_new_devices = True, *a, **k):
            pass


    class PrEditorParameterProvider():
        pass
...
if g_apc40:
    from _APC.DeviceComponent import DeviceComponent as Framework_DeviceComponentBase
else:
...
class DeviceComponent(Framework_DeviceComponentBase, PrEditorMapper, DeviceComponentBase, PrEditorParameterProvider):
    ...
    def __init__(self, script, num_params = 8, direct_bank = False, mono = True, shared = False, path = None, delayed_update = False, new_skin = False, *a, **k):
        self._script = script
        ...
        if not self._is_osc:
            g_hud = False
        self.log('DeviceComponent(STANDARD): num: ' + str(num_params) + ', path: ' + str(path) + ', HUD: ' + str(g_hud) + ', OSC: ' + str(self._is_osc))
        if g_mapper:
        ...
        DeviceComponentBase.__init__(self, script, PrEditorMapper if g_mapper else None, False, num_params, direct_bank, mono, shared, path, delayed_update, new_skin)
...
class DeviceModeComponent(EnablingModesComponent):
    ...
    def __init__(self, script = None, component = None, direct_bank = False, device_settings_mode = None, *a, **k):
        self._script = script
    ...
    def device_mode_button(self, button):
        ...
        if hasattr(self, '_script'):
            self._script._on_device_engage(127 if self.selected_mode == 'enabled' else 0)
    ...
    def device_mode_button(self, button):
        ...
        if hasattr(self, '_script') and hasattr(self._script, 'device_mode_pressed_delayed'):
            self._script.device_mode_pressed_delayed()

    @device_mode_button.released_delayed
    def device_mode_button(self, button):
        self.log('DeviceModeComponent:released_delayed')
        if hasattr(self, '_script') and hasattr(self._script, 'device_mode_released_delayed'):
            self._script.device_mode_released_delayed()
