...
from consts import *
import sys
from MidiKontrolHelper import MidiKontrolHelper
from ParamMap import ParamMap
from Devices import *
from Tracing import Traced
from DumpXML import dumpXML

class MidiKontrolScript(Traced):
#class MidiKontrolScript:
...
#    __myDebug__ = False
    __myDebug__ = True
    __version__ = "V1.1"
    __name__ = "Generic MidiKontrol Script"
	...
	if self.__myDebug__:
            if sys.platform == "win32":
                self.file = open("C:/MidiKontrol" + self.suffix + ".txt", "a")
                self.commandfile = "C:/MidiKontrol-cmd" + self.suffix + ".txt"
            else:
                self.file = open("/tmp/MidiKontrol" + self.suffix, "a")
                self.commandfile = ("/tmp/MidiKontrol-cmd" + self.suffix)
	    self.log("MidiKontrol")
	...
	self.helper = MidiKontrolHelper(self)
